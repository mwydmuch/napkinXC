...
if [[ ! -e $TEST_RESULT_FILE ]] || [[ -e $TEST_LOCK_FILE ]]; then
    ...
    for i in $(seq 0 1 20)
    do
        PRED_FILE_IT=${PRED_FILE}_${i}
        if [[ -e $PRED_FILE_IT ]]; then
            echo $PRED_FILE_IT | tee -a $TEST_RESULT_FILE
            python3 ${SCRIPT_DIR}/evaluate.py $TEST_FILE $PRED_FILE_IT $INV_PS_FILE | tee -a $TEST_RESULT_FILE
        fi
    done
