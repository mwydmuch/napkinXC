...
void solve_l2r_lr_dual(const problem *prob, float *w, float eps, float Cp, float Cn, int max_iter)
{
	...
	while (iter < max_iter)
	{
		...
		for (s=0; s<l; s++)
		{
			...
			if(0.5f*a*(alpha[ind2]-alpha[ind1])+b < 0)
			{
				ind1 = 2*i+1;
				ind2 = 2*i;
				sign = -1;
		...
		if(newton_iter <= l/10)
			innereps = max(innereps_min, 0.1f*innereps);
	...
	*best_C = start_C;
	...
	while(param_tmp->C <= max_C)
	{
		...
		for(i=0; i<nr_fold; i++)
		{
			...
			struct model *submodel = train_liblinear(&subprob[i],param_tmp);
...
// Interface functions
//
model* train_liblinear(const problem *prob, const parameter *param)
{
	problem newprob;
...
void cross_validation(const problem *prob, const parameter *param, int nr_fold, float *target)
{
	...
	for(i=0;i<nr_fold;i++)
	{
		...
		struct model *submodel = train_liblinear(&subprob,param);
		for(j=begin;j<end;j++)
			target[perm[j]] = predict(submodel,prob->x[perm[j]]);
		free_and_destroy_model(&submodel);
		free(subprob.x);
