...
U=("uP --alfa 1.0" "uAlfaBeta --alfa 1.0 --beta 0.1" "uAlfaBeta --alfa 1.0 --beta 0.5" "uAlfaBeta --alfa 1.0 --beta 1.0")
...
for u in "${U[@]}"; do
    ...
    if [ ! -e ${OUTPUT}_test_${full_u} ]; then
        (time nxml test -i $TEST_FILE -o $MODEL -t $THREADS --header 0 --setUtility ${base_u} ${u_args} --measures "p@1,${base_u}") > ${OUTPUT}_test_${full_u} 2>&1
    fi
