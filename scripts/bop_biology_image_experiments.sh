...
for d in "${DATASETS[@]}"; do
    ...
    bash testBOP.sh "HIERARCHICAL_PREDEFINED_${d}" -m ubopch --treeStructure "data/HIERARCHICAL_PREDEFINED_${d}/HIERARCHICAL_PREDEFINED_${d}.hier" ${ARGS}
    bash testBOP.sh "HIERARCHICAL_RANDOM_${d}" -m ubopch --treeStructure "data/HIERARCHICAL_RANDOM_${d}/HIERARCHICAL_RANDOM_${d}.hier" ${ARGS}

    bash testBOP.sh "FLAT_${d}" -m ubop ${ARGS}

    # UBOP-CH on flat hidden representation
    #bash testBOP.sh "FLAT_${d}" -m ubopch --treeType hierarchicalKMeans ${ARGS}
