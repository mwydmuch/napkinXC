	...
	*best_C = start_C;
	...
	while(param_tmp->C <= max_C)
	{
		...
		for(i=0; i<nr_fold; i++)
		{
			...
			struct model *submodel = train_linear(&subprob[i],param_tmp);
...
// Interface functions
//
model* train_linear(const problem *prob, const parameter *param)
{
	int i,j;
	int l = prob->l;
	int n = prob->n;
...
void cross_validation(const problem *prob, const parameter *param, int nr_fold, double *target)
{
	...
	for(i=0;i<nr_fold;i++)
	{
		...
		struct model *submodel = train_linear(&subprob,param);
		for(j=begin;j<end;j++)
			target[perm[j]] = predict(submodel,prob->x[perm[j]]);
		free_and_destroy_model(&submodel);
		free(subprob.x);
