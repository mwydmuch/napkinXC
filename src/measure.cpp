...
std::vector<std::shared_ptr<Measure>> Measure::factory(Args& args, int outputSize) {
    ...
    for (const auto& m : measuresNames) {
        ...
        if (mAt.size() > 1) {
            ...
            if (mAt[0] == "p")
                measures.push_back(
                    std::static_pointer_cast<Measure>(std::make_shared<PrecisionAtK>(args, outputSize, k)));
            else if (mAt[0] == "r")
                measures.push_back(std::static_pointer_cast<Measure>(std::make_shared<RecallAtK>(args, outputSize, k)));
            else if (mAt[0] == "c")
                measures.push_back(
                    std::static_pointer_cast<Measure>(std::make_shared<CoverageAtK>(args, outputSize, k)));
            else
            ...
            if (m == "p")
                ...
                measures.push_back(
                    std::static_pointer_cast<Measure>(std::make_shared<PrecisionUtility>(args, outputSize)));
            else if (m == "uF1")
                ...
                measures.push_back(
                    std::static_pointer_cast<Measure>(std::make_shared<UtilityAlfaBeta>(args, outputSize)));
...
MeasureAtK::MeasureAtK(Args& args, int outputSize, int k) : Measure(args, outputSize) { this->k = k; }

Recall::Recall(Args& args, int outputSize) : Measure(args, outputSize) { name = "Recall"; }
...
Precision::Precision(Args& args, int outputSize) : Measure(args, outputSize) { name = "Precision"; }
...
Accuracy::Accuracy(Args& args, int outputSize) : Measure(args, outputSize) { name = "Acc"; }

void Accuracy::accumulate(Label* labels, const std::vector<Prediction>& prediction) {
    if (labels[0] > -1 && prediction.size() && labels[0] == prediction[0].label) ++sum;
    ++count;
}
