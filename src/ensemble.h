...
template <typename T>
void Ensemble<T>::accumulatePrediction(
...
std::vector<std::vector<Prediction>> Ensemble<T>::predictBatch(SRMatrix<Feature>& features, Args &args){
    if(!args.onTheTrotPrediction)
        return Model::predictBatch(features, args);
    ...
    for(int memberNo = 0; memberNo < args.ensemble; ++memberNo) {
        ...
        for(int i = 0; i < rows; ++i)
            accumulatePrediction(ensemblePredictions[i], memberPredictions[i], memberNo);
...
template <typename T>
T* Ensemble<T>::loadMember(Args& args, const std::string& infile, int memberNo) {
